...
// Adjust the following lines to constrain the shell within the screen boundaries
shell.x = Math.max(0, Math.min(1, shell.x));
shell.y = Math.max(0, Math.min(1, shell.y));

shell.xoff -= (shell.xoff * dt * 0.001);
shell.yoff -= ((shell.yoff + 0.2) * dt * 0.00005);

// Check if the shell has reached a certain height, generate passes, and remove the shell
if (shell.y < 0 || shell.yoff < -0.005) {
  newPass(shell);
  shells.splice(ix, 1);
}
